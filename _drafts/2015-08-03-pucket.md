...
Take me to [tldr](#tldr)


Pucket is a partitioning system for Parquet. But what is Parquet and why does it need partitioning?
...
The is a little bit vague, so let me try to answer the important question, why do want to use Parquet to store data in Hadoop? Parquet stores data as columnar records in files. Columns in this case are essentially nested key values with strongly typed primitive values. Parquet also encourages compressing of the dataset, as the data format promotes compression, doing this will reduce your storage footprint and increase your IO throughput at the cost of CPU cycles[^2]. Parquet files contain metadata which promote efficient seeking within the data[^3]. Both of these properties combined enable Parquet to have a small storage footprint and quick access time when compared to simple sequence files.
...
* Functionally orientated - Minimal side effects and no mutable state
...
Pucket aims to provide the following functionality:
...
This is a partially implemented trait which contains information on the the data in a certain directory (or bucket) on the filesystem. It has a few simple operations:
...
We have been heavy users of [Pail](https://github.com/nathanmarz/dfs-datastores) in the past for partitioning our data, however this project is rarely updated and appears to be abondonware, also it uses version 1 of the MapReduce api. So why not modify it to work with Parquet?
...
The Scalaz implementation of `Either`, known as disjunction[^4] (`\/`), is used heavily within the Pucket code, this is a proper Monad which allows the results to be used in a flat map or for comprehension. This allows a happy path and sad path to be accounted for when performing any side-effecting operation. Every operation requiring interaction with the Hadoop filesystem will return a disjunction.
...
import your.pucket.Partitioner
...
val PucketDescriptor: ThriftPucketDescriptor[ThriftData] =
...
val newPucket: Throwable \/ Pucket[ThriftData] =
  ThriftPucket.create[ThriftData](path, fs, PucketDescriptor)
...
val existingPucket: Throwable \/ Pucket[ThriftData] =
...
val maybeExistingPucket: Throwable \/ Pucket[ThriftData] =
  ThriftPucket.findOrCreate[ThriftData](path, fs, PucketDescriptor)
{% endhighlight %}
...
def write[Ex](data: Seq[ThriftData],
              writer:  Ex \/ Writer[ThriftData, Ex]): Ex \/ Writer[ThriftData, Ex] =
...
{% highlight scala %}
def writeMeSomeData(data: Seq[ThriftData],
                    Pucket: Throwable \/ Pucket[ThriftData]): Throwable \/ Unit =
  for {
    p <- pucket
    writer <- p.writer
...
{% highlight scala %}
def writeMeSomeDataIncrementally(data: Seq[ThriftData],
                                 Pucket: Throwable \/ Pucket[ThriftData]): (Long, Throwable) \/ Unit =
  for {
...
{% highlight scala %}
def writeMeSomePartitionedData(data: Seq[ThriftData],
                               Pucket: Throwable \/ Pucket[ThriftData]): Throwable \/ Unit =
  for {
    p <- pucket
...
{% highlight scala %}
def writeMeSomePartitionedDataIncrementally(data: Seq[ThriftData],
                                            Pucket: Throwable \/ Pucket[ThriftData]): (Long, Throwable) \/ Unit =
  for {
    p <- pucket.leftMap((0, _))
...
The reader behaves in a similar way to the writer in that each read returns a new instance of the reader with updated state, however as well as a new reader instance, an option of the data item is returned.

![Nic Cage](http://nic-cage.xyz)
